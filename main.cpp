...
int main (void)
{
    ...
    if ((res = InsertAfter(&lst, 0, 50)) < 0)
        OUTPUT_ERROR_AND_FINISH;
    if ((res = InsertAfter(&lst, 0, 60)) < 0)
        OUTPUT_ERROR_AND_FINISH;
    Linearise(&lst);
    GraphDump(&lst, After, "Linearise", __FILE__, __LINE__, 0, 0, SUCCESS, "Проверочный вызов\n");
    Optimise(&lst);
    GraphDump(&lst, After, "Optimise", __FILE__, __LINE__, 0, 0, SUCCESS, "Проверочный вызов\n");
